...
| 本番広告ID/SDK結線・ATT/UMP同意 | 未着手（所有者側） |
| ReplayEngine / 診断 | 未着手 |
...
- [x] 購入画面UI（`PurchaseView`＋`PurchaseViewModel`：購入/復元/エンティトルメント表示・広告なし・`PurchaseViewModelTests`＋UIスモーク） → [ ] 本番広告ID/SDK結線・ATT/UMP同意（所有者側）
...
- [ ] WeatherKit 帰属表示 UI・権限拒否時の限定モード UI（後続）
