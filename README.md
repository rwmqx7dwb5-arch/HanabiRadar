...
| `HanabiCore` 気象/標高プロバイダ契約＋反復気象補正＋地上高解決（§13 の MSL/相対/地上高の区別） | **実装済み（新規・Swift は要CI確認）** |
| Python 参照オラクル＋クロス検証fixture（§24.2・音速/測地/レイ/位置復元を独立実装で照合） | **実装済み・ローカルで数理検証済み（Swift 側照合は要CI）** |
| WeatherKit 実アダプタ（app 側 `WeatherKitProvider`） | **実装済み（要CIビルド確認・実行は entitlement+実機=所有者側）** |
| DEM 標高アダプタ（app 側の具体実装。`ElevationProviding` 契約はコアに実装済み） | 未着手 |
| SwiftUI 画面（測定・結果・地図・履歴・設定・購入） | 骨組み＋結果画面デモ（`ResultView`）。誠実な表示フォーマットは `FormattingTests`、画面遷移は UIスモークで検証（要CIビルド） |
