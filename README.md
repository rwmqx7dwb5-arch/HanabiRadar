...
| 購入画面UI・本番広告ID/SDK結線・ATT/UMP同意 | 未着手（所有者側） |
| ReplayEngine / 診断 | 未着手 |
| Xcode アプリプロジェクト（`project.yml` → XcodeGen） | **骨組み実装済み・CI緑** |
| iOS Simulator ビルド/テストCI（署名なし・デバイス動的解決） | **緑（build＋Simulator上の単体テスト）** |
| App Icon（夜空＋花火・1024 不透過）・Accent Color（Light/Dark） | **実装済み**（`Assets.xcassets`・`project.yml` 結線・要 iOS CI） |
...
| Info.plist 権限説明文の EN ローカライズ／UI 文字列のカタログ化 | EN InfoPlist.strings **実装済み**・UI 文字列 `Localizable.xcstrings` は後続 |
...
- [x] App Icon（夜空＋花火・1024 不透過 RGB）＋ Accent Color（Light/Dark）＋ `project.yml` 結線
...
- [ ] アプリ内 UI 文字列のカタログ化（`Localizable.xcstrings`）・アクセシビリティ仕上げ（後続）
- [ ] WeatherKit 帰属表示 UI・購入画面 UI・権限拒否時の限定モード UI（後続）
