...
- **App Icon / Accent**: **ユーザー提供**の夜空＋花火デザインを採用（元画像 1254px の黒縁を除去し 1024 不透過 RGB へ
  ...
  **要確認事項**: actool による実ビルドと実機/シミュレータでの見え方（マスク後の角・小サイズ視認性）は所有者側で確認する。
  （アイコン内表記の綴りミス「Hanabi Rader」→正しい「Hanabi Radar」は Phase 6l で修正済み。ラスターの当該 1 文字
  のみを既存グリフの複製で差し替え、デザインは不変。正式名称は「Hanabi Radar」で確定。）
